...
public class LockCheckIfElseRecipe extends Recipe {
    ...
    public String getDisplayName() {
        return "Check Lock() in IfElseRecipe";
    }

    @Override
    protected JavaIsoVisitor<ExecutionContext> getVisitor() {
        // getVisitor() should always return a new instance of the visitor to avoid any state leaking between cycles
        return new LockCheckIfElseVisitor();
    }

    public class LockCheckIfElseVisitor extends JavaIsoVisitor<ExecutionContext> {
        ...
        @Override
        public J.If visitIf(J.If ifelse, ExecutionContext c){
            J.If ie = super.visitIf(ifelse, c);

            // Check if the ifelse contains lock
            boolean lockMethodExists = ((J.Block)ie.getThenPart()).getStatements().stream()
            ...
            //collect all the "Try"
            List<J.Try> tryList = ((J.Block)ie.getThenPart()).getStatements().stream()
            ...
            List<J.MethodInvocation> lockList = ((J.Block)ie.getThenPart()).getStatements().stream()
                    .filter(statement -> statement instanceof J.MethodInvocation)
                    .map(J.MethodInvocation.class::cast)
            ...
            List<J.MethodInvocation> unlockList = ((J.Block)ie.getThenPart()).getStatements().stream()
                    .filter(statement -> statement instanceof J.MethodInvocation)
                    .map(J.MethodInvocation.class::cast)
                ...
                if(typeExist){
                    ...
                    if(unlockMap.get(locks.getSelect().print())==null||unlockMap.get(locks.getSelect().print())<map.get(locks.getSelect().print())){
                        if(isTry){
                            ...
                            ie = ie.withTemplate(
                                    unLockTemplate,
                                    ((J.Block)ie.getThenPart()).getCoordinates().lastStatement(),
                                    locks.getSelect().print()
                            );
                        }
